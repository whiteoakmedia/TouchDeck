...
static void TUCPostKeyEvent(CGKeyCode keyCode, bool down, CGEventFlags flags) {
...
- (void)postKeystroke:(CGKeyCode)keyCode flags:(CGEventFlags)flags {
    // Press and release the modifiers as real key events. Cmd+Tab in particular engages the
    // app switcher on cmd-down and only completes when it sees cmd released again.
    static const struct { CGEventFlags mask; CGKeyCode key; } modifiers[] = {
        { kCGEventFlagMaskCommand,   55 },
        { kCGEventFlagMaskControl,   59 },
        { kCGEventFlagMaskAlternate, 58 },
        { kCGEventFlagMaskShift,     56 },
    };
    static const int modifierCount = sizeof(modifiers) / sizeof(modifiers[0]);
    
    CGEventFlags current = 0;
    for (int i = 0; i < modifierCount; i++) {
        if (flags & modifiers[i].mask) {
            current |= modifiers[i].mask;
            TUCPostKeyEvent(modifiers[i].key, true, current);
        }
    }
    
    // Arrow keys on real keyboards always carry the Fn + NumericPad flags, and the
    // symbolic hotkeys (ctrl+arrow = Mission Control etc.) are registered with them —
    // a bare ctrl+arrow event is silently ignored by the hotkey matcher.
    CGEventFlags keyFlags = current;
    if (keyCode >= 123 && keyCode <= 126) {
        keyFlags |= kCGEventFlagMaskSecondaryFn | kCGEventFlagMaskNumericPad;
    }
    
    TUCPostKeyEvent(keyCode, true, keyFlags);
    TUCPostKeyEvent(keyCode, false, keyFlags);
    
    for (int i = modifierCount - 1; i >= 0; i--) {
        if (flags & modifiers[i].mask) {
            current &= ~modifiers[i].mask;
            TUCPostKeyEvent(modifiers[i].key, false, current);
        }
    }
}
